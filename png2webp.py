...
## About
## A fancy Script for converting images to webp
...
def optimize(in_file: str):
    out_file = change_filetype(in_file, 'webp')
    commands = f'cwebp -z 9 "{in_file}" -o "{out_file}"'
    p = subprocess.run(args=commands, shell=True, capture_output=True, text=True)
    if p.returncode != 0:
        raise Exception(f'cwebp returned with non 0 return code ({p.returncode})\n{p}')
    os.remove(in_file)
